...
def embed_seq(
    ...
    with tf.variable_scope("embedding"):  # embed + BN input set
        ...
        embedded_input = tf.nn.conv1d(
            input_seq, W_embed, 1, "VALID", name="embedded_input"
        )
        if BN == True:
...
def multihead_attention(
    inputs, num_units=None, num_heads=16, dropout_rate=0.1, is_training=True
):
    with tf.variable_scope("multihead_attention", reuse=None):
        ...
        )  # [batch_size, seq_length, n_hidden/num_heads]
        K_ = tf.concat(
        ...
        # Multiplication
        outputs = tf.matmul(
            Q_, tf.transpose(K_, [0, 2, 1])
        )  # num_heads*[batch_size, seq_length, seq_length]
        # Scale
        ...
        outputs = tf.matmul(
            outputs, V_
        )  # num_heads*[batch_size, seq_length, n_hidden/num_heads]
        # Restore shape
        outputs = tf.concat(
        ...
        outputs = tf.layers.batch_normalization(
            outputs, axis=2, training=is_training, name="ln", reuse=None
        )  # [batch_size, seq_length, n_hidden]
    ...
    with tf.variable_scope("stack"):
        for i in range(num_stacks):  # block i
            with tf.variable_scope(
                ...
                input_seq = feedforward(
                    input_seq,
                    num_units=[num_neurons, input_dim],
                    is_training=is_training,
                )
...
def pointer(encoded_ref, query, mask, W_ref, W_q, v, C=10.0, temperature=1.0):
    encoded_query = tf.expand_dims(
        tf.matmul(query, W_q), 1
    )  # [Batch size, 1, n_hidden]
    scores = tf.reduce_sum(
        v * tf.tanh(encoded_ref + encoded_query), [-1]
    )  # [Batch size, seq_length]
    scores = C * tf.tanh(scores / temperature)  # control entropy
    masked_scores = tf.clip_by_value(
        scores - 100000000.0 * mask, -100000000.0, 100000000.0
    )  # [Batch size, seq_length]
...
def full_glimpse(ref, from_, to_, initializer=tf.contrib.layers.xavier_initializer()):
    with tf.variable_scope("glimpse"):
        ...
        # Attending mechanism
        encoded_ref_g = tf.nn.conv1d(
            ref, W_ref_g, 1, "VALID", name="encoded_ref_g"
        )  # [Batch size, seq_length, n_hidden]
        scores_g = tf.reduce_sum(
            v_g * tf.tanh(encoded_ref_g), [-1], name="scores_g"
        )  # [Batch size, seq_length]
