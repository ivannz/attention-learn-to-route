...
input_batch = dataset.test_batch(
    batch_size=128, max_length=50, dimension=2, seed=123
)  # Generate some data
...
data_arg.add_argument(
    "--max_length", type=int, default=50, help="number of cities"
)  ##### #####
...
net_arg.add_argument(
    "--input_embed", type=int, default=128, help="actor critic input embedding"
)
net_arg.add_argument(
    "--num_neurons", type=int, default=512, help="encoder inner layer neurons"
)
...
net_arg.add_argument(
    "--query_dim", type=int, default=360, help="decoder query space dimension"
)
net_arg.add_argument(
    "--num_units",
    type=int,
    default=256,
    help="decoder and critic attention product space",
)
net_arg.add_argument(
    "--num_neurons_critic", type=int, default=256, help="critic n-1 layer"
)
...
train_arg.add_argument("--init_B", type=float, default=7.0, help="critic init baseline")
train_arg.add_argument(
    "--lr_start", type=float, default=0.001, help="actor learning rate"
)
train_arg.add_argument("--lr_decay_step", type=int, default=5000, help="lr1 decay step")
train_arg.add_argument(
    "--lr_decay_rate", type=float, default=0.96, help="lr1 decay rate"
)
train_arg.add_argument(
    "--temperature", type=float, default=1.0, help="pointer initial temperature"
)
...
dir_ = (
    ...
    + "_e"
    + str(config.input_embed)
    + "_n"
    + str(config.num_neurons)
    + "_s"
    + str(config.num_stacks)
    + "_h"
    + str(config.num_heads)
    + "_q"
    + str(config.query_dim)
    + "_u"
    + str(config.num_units)
    + "_c"
    + str(config.num_neurons_critic)
    + "_lr"
    + str(config.lr_start)
    + "_d"
    + str(config.lr_decay_step)
    + "_"
    + str(config.lr_decay_rate)
    + "_T"
...
class Actor(object):
    def __init__(self):
        ...
        self.num_units = (
            config.num_units
        )  # dimension of attention product space (decoder and critic)
        self.num_neurons_critic = (
            config.num_neurons_critic
        )  # critic n-1 layer num neurons
        self.initializer = (
        ...
        self.global_step2 = tf.Variable(
            0, trainable=False, name="global_step2"
        )  # critic global step
    ...
    def encode_decode(self):
        ...
        W_ref = tf.get_variable(
            "W_ref", [1, n_hidden, self.num_units], initializer=self.initializer
        )
        W_q = tf.get_variable(
            "W_q", [self.query_dim, self.num_units], initializer=self.initializer
        )
        ...
        W_1 = tf.get_variable(
            "W_1", [n_hidden, self.query_dim], initializer=self.initializer
        )  # update trajectory (state)
        W_2 = tf.get_variable(
            "W_2", [n_hidden, self.query_dim], initializer=self.initializer
        )
        W_3 = tf.get_variable(
            "W_3", [n_hidden, self.query_dim], initializer=self.initializer
        )

        for step in range(self.max_length):  # sample from POINTER
            query = tf.nn.relu(
                tf.matmul(query1, W_1) + tf.matmul(query2, W_2) + tf.matmul(query3, W_3)
            )
            logits = pointer(
            ...
            idx_ = tf.stack(
                [tf.range(self.batch_size, dtype=tf.int32), idx], 1
            )  # idx with batch
            query3 = query2
            query2 = query1
        ...
        self.log_prob = tf.add_n(
            log_probs
        )  # corresponding log-probability for backprop
    ...
    def build_reward(
        self,
    ):  # reorder input % tour and return tour length (euclidean distance)
        self.permutations = tf.stack(
            [
                tf.tile(
                    tf.expand_dims(tf.range(self.batch_size, dtype=tf.int32), 1),
                    [1, self.max_length + 1],
                ),
                self.tour,
            ],
        ...
        if self.is_training == True:
            ...
            self.ordered_input_ = tf.gather_nd(
                tf.tile(self.input_, [self.batch_size, 1, 1]), self.permutations
            )
        self.ordered_input_ = tf.transpose(
            self.ordered_input_, [2, 1, 0]
        )  # [features, seq length +1, batch_size]   Rq: +1 because end = start

        ordered_x_ = self.ordered_input_[
            0
        ]  # ordered x, y coordinates [seq length +1, batch_size]
        ordered_y_ = self.ordered_input_[
            1
        ]  # ordered y coordinates [seq length +1, batch_size]
        delta_x2 = tf.transpose(
            tf.square(ordered_x_[1:] - ordered_x_[:-1]), [1, 0]
        )  # [batch_size, seq length]        delta_x**2
        delta_y2 = tf.transpose(
            tf.square(ordered_y_[1:] - ordered_y_[:-1]), [1, 0]
    ...
    def build_critic(self):
        ...
        critic_encoding = encode_seq(
            input_seq=critic_embedding,
            input_dim=self.input_embed,
            num_stacks=self.num_stacks,
            num_heads=self.num_heads,
            num_neurons=self.num_neurons,
        ...
        with tf.variable_scope("ffn"):  #  2 dense layers for predictions
            ...
            w1 = tf.get_variable(
                "w1", [self.num_neurons_critic, 1], initializer=self.initializer
            )
    ...
    def build_optim(self):
        ...
        with tf.control_dependencies(
            update_ops
        ):  # Update moving_mean and moving_variance for BN

            with tf.name_scope("reinforce"):
                lr1 = tf.train.natural_exp_decay(
                ...
                self.loss = tf.reduce_mean(
                    tf.stop_gradient(self.reward - self.predictions) * self.log_prob,
                    axis=0,
                )  # loss actor
                ...
                self.trn_op1 = opt1.apply_gradients(
                    grads_and_vars=capped_gvs1, global_step=self.global_step
                )  # minimize op actor

            with tf.name_scope("state_value"):
                lr2 = tf.train.natural_exp_decay(
                    learning_rate=self.lr_start,
                ...
                self.trn_op2 = opt2.apply_gradients(
                    grads_and_vars=capped_gvs2, global_step=self.global_step2
                )  # minimize op critic
...
with tf.Session() as sess:  # start session
    ...
    for epoch in range(config.nb_epochs):
        ...
        for i in tqdm(
            range(config.nb_steps), mininterval=30
        ):  # Forward pass & train step
            input_batch = dataset.train_batch(
                actor.batch_size, actor.max_length, actor.dimension
            )
        ...
        if epoch % 10 == 0:
            saver.save(
                sess, save_path + "/actor{epoch}.ckpt"
...
def eval_dataset(ds, greedy, num_samples, save_path, max_length, seed=123):
    ...
    saver = tf.train.Saver(
        var_list=variables_to_save, keep_checkpoint_every_n_hours=1.0
    )
    ...
    with tf.Session() as sess:  # start session
        ...
        for i, coords in enumerate(tqdm(ds, mininterval=30)):
            ...
            feed = {actor.input_: input_batch}  # Get feed dict
            tour, reward = sess.run(
                [actor.tour, actor.reward], feed_dict=feed
            )  # sample tours
            ...
            results.append(
                (
                    calc_tsp_length(coords, best_permutation),
                    best_permutation,
                    time.time() - start,
                )
            )
            ...
            opt_tour_ind = np.linalg.norm(
                opt_tour[:, None, :] - input_batch[0][None, :, :], axis=-1
            ).argmin(-1)

            results_w2opt.append(
                (
                    calc_tsp_length(coords, opt_tour_ind),
                    opt_tour_ind,
                    time.time() - start,
                )
            )
        ...
        print(
            "Testing COMPLETED ! Mean length1:",
            np.mean(predictions_length),
            "Mean length2:",
...
def save_results(size, method, results, parallelism=1):
    ...
    out_file = os.path.join(
        results_dir, "tsp{}_test_seed1234-{}.pkl".format(size, method)
    )
...
for test_instance_size in config.n_test:
    ...
    results_greedy, results_greedy_2opt = eval_dataset(
        dataset_test, True, 1, save_path, test_instance_size
    )
    save_results(test_instance_size, f"deudon{train_size}greedy", results_greedy)
    save_results(
        test_instance_size, f"deudon{train_size}greedy2opt", results_greedy_2opt
    )
    results_sampling, results_sampling_2opt = eval_dataset(
        dataset_test, False, 1280, save_path, test_instance_size
    )
    save_results(test_instance_size, f"deudon{train_size}sampling", results_sampling)
    save_results(
        test_instance_size, f"deudon{train_size}sampling2opt", results_sampling_2opt
    )
