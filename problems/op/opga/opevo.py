...
def fitness(chrom, s, start_point, end_point, tmax):
    ...
    for i in range(len(s)):
        augs.append((s[i][0], s[i][1], s[i][2], s[i][3], s[i][4] + chrom[i]))
    if debug:
        print("fitness---------------------------------")
        print("augs:")
        print(augs)
...
def mutate(chrom, mchance, msigma):
    return [
        x + random.gauss(0, msigma) if random.randrange(mchance) == 0 else x
        for x in chrom
    ]
...
def run_alg(points, tmax, return_sol=False, verbose=True):
    ...
    for i in range(genlimit):
        ...
        for j in range(popsize):
            ...
            offspring = (
                fitness(offspring, cpoints, start_point, end_point, tmax)[0],
                offspring,
            )
    ...
    if verbose:
        ...
        print("their stuff:")
    stuff = oph.initialize(
        oph.ell_sub(tmax, start_point, end_point, cpoints), start_point, end_point, tmax
    )[0]
    if verbose:
    ...
    total_distance = oph.distance(
        start_point, cpoints[best_path[1][3] - 2]
    ) + oph.distance(end_point, cpoints[best_path[len(best_path) - 2][3] - 2])
    for i in range(1, len(best_path) - 3):
        total_distance += oph.distance(
            cpoints[best_path[i][3] - 2], cpoints[best_path[i + 1][3] - 2]
        )
    if verbose:
