...
class OP(object):
    ...
    def get_costs(dataset, pi):
        ...
        prize_with_depot = torch.cat(
            (torch.zeros_like(dataset["prize"][:, :1]), dataset["prize"]), 1
        )
        ...
        assert (
            length <= dataset["max_length"] + 1e-5
        ).all(), "Max length exceeded by {}".format(
            (length - dataset["max_length"]).max()
        )
...
def generate_instance(size, prize_type):
    ...
    if prize_type == "const":
        ...
        prize = (1 + torch.randint(0, 100, size=(size,))) / 100.0
        ...
        if filename is not None:
            ...
            with open(filename, "rb") as f:
                ...
                self.data = [
                    ...
                    for depot, loc, prize, max_length in (
                        data[offset : offset + num_samples]
                    )
                ]
        else:
            self.data = [
                generate_instance(size, prize_type) for i in range(num_samples)
            ]
