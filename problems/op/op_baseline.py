...
def solve_compass_log(
    executable, directory, name, depot, loc, prize, max_length, disable_cache=False
):
    ...
    try:
        ...
        if os.path.isfile(output_filename) and not disable_cache:
            ...
            if not calc_op_length(depot, loc, tour) <= max_length:
                print(
                    "Warning: length exceeds max length:",
                    calc_op_length(depot, loc, tour),
                    max_length,
                )
            assert (
                calc_op_length(depot, loc, tour) <= max_length + MAX_LENGTH_TOL
            ), "Tour exceeds max_length!"
...
def write_oplib(filename, depot, loc, prize, max_length, name="problem"):
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            "\n".join(["{}\t{}".format(i + 1, d) for i, d in enumerate([0] + prize)])
        )
        f.write("\n")
...
def solve_ortools(
    ...
    try:
        ...
        if os.path.isfile(problem_filename) and not disable_cache:
            ...
            # 0 = start, 1 = end so add depot twice
            start = time.time()
            objval, tour = solve_op_ortools(
                depot, loc, prize, max_length, sec_local_search=sec_local_search
            )
            duration = time.time() - start
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "method", help="Name of the method to evaluate, 'compass', 'opga' or 'tsili'"
    )
    parser.add_argument(
            ...
            if method[:6] == "gurobi":
                ...
                def run_func(args):
                    return solve_ortools(
                        *args, sec_local_search=runs, disable_cache=opts.disable_cache
                    )
